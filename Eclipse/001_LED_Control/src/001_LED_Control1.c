...
int write_trigger_values(uint8_t led_no, char *value)
{
	...
	snprintf(buff, sizeof(buff),
			 SYS_FS_LEDS_PATH "/beaglebone:green:usr%d/trigger", led_no);
...
int write_brightness_values(uint8_t led_no, int value)
{
	int fd, ret = 0;
	char buff[SOME_BYTES];
	snprintf(buff, sizeof(buff), SYS_FS_LEDS_PATH "/beaglebone:green:usr%d/brightness", led_no);
...
{
	if (!(strcmp(value, "timer") && strcmp(value, "heartbeat") && strcmp(value, "none") && strcmp(value, "oneshot") && strcmp(value, "default-on")))
	{
